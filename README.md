...
## Note
This is a modified version of [this repo](https://github.com/sclorg/nodejs-ex) and is a work in progress.

- [TypeScript Node.js sample app on OpenShift!](#typescript-nodejs-sample-app-on-openshift)
...
## TypeScript Node.js sample app on OpenShift!
-----------------
...
After logging in with `oc login` (default username/password: openshift), if you don't have a project setup all ready, go ahead and take care of that:
        ...
        $ oc new-project nodejs-ts-echo \
        $ --display-name="nodejs-ts" --description="Sample TypeScript Node.js app"
...
Pointing `oc new-app` at source code kicks off a chain of events, for our example run:
        ...
        $ oc new-app https://github.com/evanshortiss/nodejs-ex-typescript -l name=myapp
...
_This is not tested for the TypeScript version of this repo_

We can also [create new apps using OpenShift template files](https://docs.openshift.com/enterprise/3.0/dev_guide/new_app.html#specifying-a-template). Clone the demo app source code from [GitHub repo](https://github.com/evanshortiss/nodejs-ex-typescript) (fork if you like).

        $ git clone https://github.com/evanshortiss/nodejs-ex-typescript
...
Which should return something like:
        ...
        In project nodejs (nodejs-ts-echo) on server https://10.2.2.2:8443

        svc/nodejs-ts - 172.30.108.183:8080
          dc/nodejs-ts deploys istag/nodejs-ex-typescript:latest <-
            bc/nodejs-ts builds https://github.com/evanshortiss/nodejs-ex-typescript with openshift/nodejs:0.10
...
If the build is not yet started (you can check by running `oc get builds`), start one and stream the logs with:
        ...
        $ oc start-build nodejs-ts --follow

You can alternatively leave off `--follow` and use `oc logs build/nodejs-ts-n` where *n* is the number of the build to track the output of the build.
...
This will help indicate what IP address the service is running, the default port for it to deploy at is 8080. Output should look like:
        ...
        nodejs-ex   172.30.249.251   <none>        8080/TCP   deploymentconfig=nodejs-ts,name=myapp   17m
...
This could also be accomplished by running:
        ...
        $ oc expose svc/nodejs-ts --hostname=www.example.com
...
Running `oc status` or checking the web console will reveal the address of the newly created MongoDB:
	...
	In project nodejs-ts-echo on server https://10.2.2.2:8443
	...
	http://10.2.2.2 to pod port 8080-tcp (svc/nodejs-ts)
	  dc/nodejs-ts deploys istag/nodejs-ex-typescript:latest <-
	    bc/nodejs-ts builds https://github.com/evanshortiss/nodejs-ex-typescript with openshift/nodejs:0.10
...
We need to add the environment variable `MONGO_URL` to our Node.js web app so that it will utilize our MongoDB, and enable the "Page view count" feature. Run:
        ...
        $ oc set env dc/nodejs-ts MONGO_URL='mongodb://admin:secret@172.30.0.112:27017/mongo_db'
